...
if [[ $EUID -ne 0 ]]; then
   ...
   exit 1
    ...
    status=-1
    if [[ ${pkgman} == yum ]]; then
        yum -y install cmake make pkgconf-pkg-config gcc g++ glm-devel glfw glfw-devel assimp assimp-devel SDL2 SDL2-devel SDL2_image SDL2_image-devel mesa-libGL-devel 
        if [[ $? -ne 0 ]]; then
            status=1
        else
            status=0
        ...
        apt-get -y --fix-missing install make pkg-config gcc g++ gdb libglfw3 libglfw3-dev libglm-dev libassimp-dev assimp-utils libegl1-mesa-dev libsdl2-2.0-0 libsdl2-dev libsdl2-image-2.0-0 libsdl2-image-dev --no-install-recommends libpulse0 libpulse-mainloop-glib0 libpulse-dev
        if [[ $? -ne 0 ]]; then
            status=1
        else
            status=0
